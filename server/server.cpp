...
int main(int argc, char *argv[])
{
    ...
    while (true)
    {
        ...
        PacketType(parametro, &pkt, newSockfd);
    }
...
void PacketType(parametros *parametro, PACKET* pkt, int newSockfd){

    switch(pkt->type){
        case MENSAGEM_LOGIN:
            functionLogin(parametro);
            break;

        case GET_SYNC_DIR:
            functionGetSyncDir(pkt,newSockfd);
            break;

        case LIVENESS_CHECK:
            functionLiveness(pkt,newSockfd);
            break;

        case LIST_SERVERS :
            functionListServer(newSockfd);
            break;

        case ID_REQUEST:
            functionIdRequest(pkt,newSockfd);
            break;

        case SERVER_ITEM:
            functionServerItem(newSockfd);
            break;

        case ELECTION:
            functionElection(pkt,newSockfd);
            break;

        case ELECTED:
            functionElected(newSockfd);
            break;
...
void functionIdRequest(PACKET * pkt, int newSockfd){
    if (!main_server)
    {
...
void functionGetSyncDir(PACKET * pkt, int newSockfd){
...
void functionElection(PACKET* pkt, int newSockfd ){
...
void functionElected(int newSockfd){
